...
public class QLTTPhieuXuat extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case android.R.id.home:
                ...
                return true;
            case R.id.menu1:
                Intent intent = new Intent(QLTTPhieuXuat.this, MainActivity.class);
    ...
    private void actionBar() {
        ...
        actionBar.setTitle("THÔNG TIN PHIẾU XUẤT");
    ...
    private void readData1() {
        ...
        tv_ds.setText("Danh sách vật tư xuất kho");
        System.out.println(soPhieu + " " + maKho);
        Cursor cursor = database.rawQuery("SELECT VATTU.MAVT , VATTU.TENVT, VATTU.HINH, CHITIETPHIEUXUAT.SOLUONG " +
                "FROM PHIEUXUAT " +
                "INNER JOIN CHITIETPHIEUXUAT ON PHIEUXUAT.SOPHIEU = CHITIETPHIEUXUAT.SOPHIEU " +
                "INNER JOIN VATTU ON CHITIETPHIEUXUAT.MAVT = VATTU.MAVT " +
                "WHERE CHITIETPHIEUXUAT.SOPHIEU = ? AND PHIEUXUAT.MAKHO = ? " +
                "ORDER BY CHITIETPHIEUXUAT.SOLUONG DESC",new String[]{soPhieu,maKho});
